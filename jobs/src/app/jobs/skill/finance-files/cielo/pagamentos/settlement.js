    ...
    for (let i = 0; i < parsed.length; i++) {
      ...
      if ((registro == '2') && (status_lancamento == 'pago')) {
        let filterResumo = {
            ...
            commissionAcctId = _.get(accountingItem, 'commission', ''),
            amount = _.get(model, 'vl_liquido', 0),
            factor = (amount < 0 ? -1 : 1),
            data = {
              id: md5(`cielo-04-pagamentos-${modelId}-${installment}`),
        ...
        if (_.isEmpty(transactionId) == false) {
          ...
          if (_.isEmpty(commissionAcctId) == false) {
            let commData = _.cloneDeep(data),
              vlComissao = _.get(model, 'vl_comissao', 0);
            commData['id'] = md5(`cielo-04-pagamentos-comissao-${modelId}-${installment}`);
            commData['transType'] = 'taxa_administracao';
            commData['accountingItemId'] = commissionAcctId;
