...
const parserRecord1 = (params) => {
    let result = {},
        ...
        ind_revenda = _.trim(line.substr(138, 1)),
    ...
    _.set(result, 'ind_revenda', getIdTable('ind_revenda', ind_revenda))
    ...
    _.set(result, 'ind_antecipacao', getIdTable('ind_antecipacao', _.trim(line.substr(160, 1))))
    ...
    if ((tipo_transacao == '01') && (ind_revenda == '')) {
        result['status_lancamento'] = 'capturado';
    }
...
const parserRecord2 = (params, parsed) => {
    ...
    let filterResumo = {
        ...
        idRejeicao = _.get(result, 'motivo_rejeicao.code', ''),
        vl_transacao = 0;
    ...
    if (resumo) {
        ...
        result['dt_apresentacao'] = _.get(resumo, 'dt_apresentacao', '');
        result['bandeira'] = _.get(resumo, 'bandeira', '');
        result['status_lancamento'] = _.get(resumo, 'status_lancamento', '');
    }
    if (_.isEmpty(idRejeicao) == false) {
        result['status_lancamento'] = 'rejeitado';
    }

    return result;
}

const parserRecordE = (params, parsed) => {
    let result = {},
        { line, fileKey, fileType, lineNumber, tags } = params,
        tipo_transacao = _.trim(line.substr(27, 2)), /// TODO: Aumentaram os tipos
        // ind_revenda = _.trim(line.substr(138, 1)), /// TODO: Nao existe
        ind_revenda = '',
        nro_terminal = _.trim(line.substr(543, 8)),
        // nro_resumo = _.trim(line.substr(187, 15)), /// TODO: Nao existe
        nro_pv = _.trim(line.substr(1, 10)),
        parcela = _.trim(line.substr(17, 2)),
        plano = _.trim(line.substr(19, 2)),
        planType = 'avista';
    ...
    /// TODO: Aumentaram os tipos de tipo de transação e não existe uma chamada PLANO CIELO
    // if (tipo_transacao == '04') {
    //     if (nro_terminal == '99999999') {
    //         line += '-' + params.totals.planoCielo;
    //         nro_terminal += '-' + params.totals.planoCielo;
    ...
    _.set(result, 'tag', getTag(tags, 'cielo', nro_pv));
    _.set(result, 'estabelecimento', nro_pv);
    _.set(result, 'nro_ro', _.trim(line.substr(597, 7)))
    _.set(result, 'nro_resumo', '') /// TODO: Não existe nro_resumo;
    _.set(result, 'parcela', parcela);
    // _.set(result, 'filler', getIdTable('filler',_.trim(line.substr(20, 1))))
    _.set(result, 'plano', plano);

    _.set(result, 'cod_bandeira', line.substr(11, 3));
    _.set(result, 'tipo_liquidacao', getIdTable('tipo_liquidacao', line.substr(14, 3)));
    _.set(result, 'cod_autorizacao', line.substr(21, 6));
    // _.set(result, 'tipo_', line.substr(27, 2)); MAPEADO ACIMA
    _.set(result, 'chave_ur', line.substr(29, 100));
    _.set(result, 'cod_transacao_recebida', line.substr(129, 22));
    _.set(result, 'cod_ajuste', line.substr(151, 4));
    _.set(result, 'forma_pagamento', line.substr(155, 3));
    _.set(result, 'ind_cielo_promo', line.substr(158, 1));
    _.set(result, 'ind_dcc', line.substr(159, 1));
    _.set(result, 'ind_comissao_minima', line.substr(160, 1));
    _.set(result, 'ind_recebimento_automatico', line.substr(161, 1));
    _.set(result, 'ind_taxa_zero', line.substr(162, 1));
    _.set(result, 'ind_transacao_rejeitada', line.substr(163, 1));
    _.set(result, 'ind_venda_tardia', line.substr(164, 1));
    _.set(result, 'bin_cartao', line.substr(165, 6));
    _.set(result, 'num_cartao', line.substr(171, 4));
    _.set(result, 'nro_cartao', line.substr(165, 6) + '******' + line.substr(171, 4));
    _.set(result, 'nro_nsu_doc', _.trim(line.substr(175, 6)));
    _.set(result, 'nro_nota_fiscal', line.substr(181, 10));
    _.set(result, 'tid', _.trim(line.substr(191, 20)));
    _.set(result, 'nro_pedido', line.substr(211, 20));
    _.set(result, 'taxa_mdr', line.substr(231, 5));
    _.set(result, 'taxa_comissao', Number(line.substr(231, 5)) / 100);
    _.set(result, 'taxa_recebimento_automatico', line.substr(236, 5));
    _.set(result, 'taxa_venda', line.substr(241, 5));
    _.set(result, 'sinal_total_venda', line.substr(246, 1));
    _.set(result, 'vl_total_venda', Number(line.substr(247, 13)) / 100);
    _.set(result, 'sinal_bruto', line.substr(260, 1));
    _.set(result, 'vl_bruto', Number(line.substr(261, 13)) / 100);
    _.set(result, 'sinal_liquido', line.substr(274, 1));
    _.set(result, 'vl_liquido', Number(line.substr(275, 13)) / 100);
    _.set(result, 'sinal_comissao', line.substr(288, 1));
    _.set(result, 'vl_comissao', Number(line.substr(289, 13)) / 100);
    _.set(result, 'sinal_comissao_minima', line.substr(302, 1));
    _.set(result, 'vl_comissao_minima', Number(line.substr(303, 13)) / 100);
    _.set(result, 'sinal_entrada', line.substr(316, 1));
    _.set(result, 'vl_entrada', Number(line.substr(317, 13)) / 100);
    _.set(result, 'sinal_tarifa_mdr', line.substr(330, 1));
    _.set(result, 'vl_tarifa_mdr', Number(line.substr(331, 13)) / 100);
    _.set(result, 'sinal_tarifa_mdr', line.substr(344, 1));
    _.set(result, 'vl_tarifa_mdr', Number(line.substr(345, 13)) / 100);
    _.set(result, 'sinal_saque', line.substr(358, 1));
    _.set(result, 'vl_saque', Number(line.substr(359, 13)) / 100);
    _.set(result, 'sinal_tarifa_embarque', line.substr(372, 1));
    _.set(result, 'vl_tarifa_embarque', Number(line.substr(373, 13)) / 100);
    _.set(result, 'sinal_pendente', line.substr(386, 1));
    _.set(result, 'vl_pendente', Number(line.substr(387, 13)) / 100);
    _.set(result, 'sinal_divida', line.substr(400, 1));
    _.set(result, 'vl_divida', Number(line.substr(401, 13)) / 100);
    _.set(result, 'sinal_cobrado', line.substr(414, 1));
    _.set(result, 'vl_cobrado', Number(line.substr(415, 13)) / 100);
    _.set(result, 'sinal_tarifa_adm', line.substr(428, 1));
    _.set(result, 'vl_tarifa_adm', Number(line.substr(429, 13)) / 100);
    _.set(result, 'sinal_cielo_promo', line.substr(442, 1));
    _.set(result, 'vl_cielo_promo', Number(line.substr(443, 13)) / 100);
    _.set(result, 'sinal_dcc', line.substr(456, 1));
    _.set(result, 'vl_dcc', Number(line.substr(457, 13)) / 100);
    _.set(result, 'hr_transacao', line.substr(470, 2) + ':' + line.substr(472, 2) + ':' + line.substr(474, 2));
    _.set(result, 'grupo_cartoes', line.substr(476, 2));
    _.set(result, 'cpf_cnpj', line.substr(478, 14));
    _.set(result, 'bandeira', getIdTable('bandeira', _.trim(line.substr(492, 3))))
    _.set(result, 'cod_unico_venda', line.substr(495, 15));
    _.set(result, 'cod_original_venda', line.substr(510, 15));
    _.set(result, 'identificador_negociacoes', line.substr(525, 15));
    _.set(result, 'canal_venda', line.substr(540, 3));
    _.set(result, 'meio_captura', getIdTable('canal_venda', _.trim(line.substr(540, 3))))
    _.set(result, 'nro_terminal', nro_terminal);
    _.set(result, 'tp_lancamento_original', line.substr(551, 2));
    // _.set(result, 'tipo_transacao', line.substr(553, 3)); /// TODO rever - Melhor pegar pelo outro tipo ja mapeado acima;
    _.set(result, 'nro_antecipacao', line.substr(556, 9));
    _.set(result, 'dt_autorizacao', line.substr(569, 4) + '-' + line.substr(567, 2) + '-' + line.substr(565, 2))
    _.set(result, 'dt_captura', line.substr(577, 4) + '-' + line.substr(575, 2) + '-' + line.substr(573, 2))
    // _.set(result, 'dt_movimento', line.substr(585, 4) + '-' + line.substr(583, 2) + '-' + line.substr(581, 2))
    _.set(result, 'dt_venda_ajuste', line.substr(585, 4) + '-' + line.substr(583, 2) + '-' + line.substr(581, 2))
    _.set(result, 'dt_movimento_original', line.substr(593, 4) + '-' + line.substr(591, 2) + '-' + line.substr(589, 2))
    _.set(result, 'nro_lote', line.substr(597, 7));
    _.set(result, 'nro_transacao', line.substr(604, 22));
    _.set(result, 'motivo_rejeicao', getIdTable('motivo_rejeicao', _.trim(line.substr(626, 3))))
    _.set(result, 'dt_vencimento_original', line.substr(633, 4) + '-' + line.substr(631, 2) + '-' + line.substr(629, 2))
    _.set(result, 'matriz_pagamento', line.substr(637, 10));
    _.set(result, 'tipo_cartao', line.substr(647, 2));
    _.set(result, 'origem_cartao', line.substr(649, 1));
    _.set(result, 'indicador_mdr_tp_cartao', line.substr(650, 1));
    _.set(result, 'ind_parcelado_cliente', line.substr(651, 1));
    _.set(result, 'banco', _.trim(line.substr(652, 4)));
    _.set(result, 'agencia', _.trim(line.substr(656, 5)));
    _.set(result, 'conta_corrente', _.trim(line.substr(661, 20 + 1)));
    _.set(result, 'arn', line.substr(682, 23));
    _.set(result, 'ind_negociacao_cielo', line.substr(705, 1));
    _.set(result, 'tipo_captura', line.substr(706, 2));
    _.set(result, 'uso_cielo', line.substr(708, 52));

    /** não esta escrito no manual mas tem o mesmo formato que a versao anterior */
    _.set(result, 'tipo_produto', getIdTable('tipo_produto', line.substr(597, 1)))

    _.set(result, '_resumo_venda', _.get(result, 'estabelecimento', '') + '-' + _.trim(line.substr(525, 15)))
    _.set(result, '_keys', getTransactionKeys(parserForKeys(result)));
    _.set(result, 'tag', getTag(tags, 'cielo', nro_pv));
    ...
    if ((['01', '02', '03'].includes(tipo_transacao)) && (ind_revenda == '')) {
            ...
            fs.readFile(fullPath, 'utf8', function (err, contents) {
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    if (type == '0') {
                        ...
                        parsed.push(parserRecordE(args, parsed));
                    }
...
export const saveData = async (params) => {
    ...
    //console.log('parsed', parsed); /// Aqui pode ser um lugar interessante para ver como preencher os campos;
    try {
        for (var i = 0; i < parsed.length; i++) {
            ...
            if (registro == '0') {
                await mongodb.cldr_cielo_vendas_record0.upsert(filter, model);
            } else if (registro == '1') {
                await mongodb.cldr_cielo_vendas_record1.upsert(filter, model);
            } else if (registro == '2') {
                await mongodb.cldr_cielo_vendas_record2.upsert(filter, model);
            } else if (registro == '3') {
                await mongodb.cldr_cielo_vendas_recordX.upsert(filter, model);
            } else if (registro == '5') {
                await mongodb.cldr_cielo_vendas_recordX.upsert(filter, model);
            } else if (registro == '6') {
                await mongodb.cldr_cielo_vendas_recordX.upsert(filter, model);
                ...
                await mongodb.cldr_cielo_vendas_record9.upsert(filter, model);
            }
        }
    } catch (error) {
        throw error;
