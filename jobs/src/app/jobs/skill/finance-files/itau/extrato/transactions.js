...
const statementParser = (model) => {
    let result = {},
        ...
        cashFlowCode = _.get(model, 'cod_fluxo_caixa.code', ''),
        cashFlowName = _.get(model, 'cod_fluxo_caixa.name', '');
    ...
    _.set(result, 'cashFlow', `${cashFlowCode}-${cashFlowName}`);
    _.set(result, 'notes', _.get(model, 'desc_historico', ''));
    ...
    _.set(result, 'sourceDb', 'cldr_itau_extrato');
