...
const parserRecord0 = (params) => {
    ...
    _.set(result, 'tipo_inscricao', line.substr(17, 1));
    _.set(result, 'nro_inscricao', line.substr(18, 14));
    _.set(result, 'complemento2', _.trim(line.substr(32, 15)));
    _.set(result, 'cod_empresa', line.substr(47, 5));
    _.set(result, 'complemento3', _.trim(line.substr(52, 1)));
    _.set(result, 'agencia', line.substr(53, 4));
    _.set(result, 'dig_agencia', line.substr(57, 1));
    _.set(result, 'complemento4', _.trim(line.substr(58, 1)));
    _.set(result, 'conta_corrente', line.substr(65, 5));
    _.set(result, 'complemento5', _.trim(line.substr(70, 1)));
    ...
    _.set(result, 'complemento6', line.substr(132, 10));
    ...
    _.set(result, 'complemento7', _.trim(line.substr(166, 5)));
...
const parserRecord1 = (params) => {
    ...
    _.set(result, 'tipo_inscricao', line.substr(17, 1));
    _.set(result, 'nro_inscricao', line.substr(18, 14));
    _.set(result, 'tipo_conta', getIdTable('tipo_conta', _.trim(line.substr(32, 4))));
    _.set(result, 'complemento2', _.trim(line.substr(36, 11)));
    _.set(result, 'cod_empresa', line.substr(47, 5));
    _.set(result, 'complemento3', _.trim(line.substr(52, 1)));
    _.set(result, 'agencia', line.substr(53, 4));
    _.set(result, 'dig_agencia', line.substr(57, 1));
    _.set(result, 'complemento4', _.trim(line.substr(58, 7)));
    _.set(result, 'conta_corrente', line.substr(65, 5));
    _.set(result, 'complemento5', _.trim(line.substr(70, 1)));
    _.set(result, 'dig_conta_corrente', line.substr(71, 1));
    _.set(result, 'nome_empresa', _.trim(line.substr(72, 30)));
    _.set(result, 'complemento6', _.trim(line.substr(102, 40)));
    ...
    _.set(result, 'moeda', line.substr(170, 3));
    _.set(result, 'nro_sequencial', line.substr(173, 5));
    _.set(result, 'complemento7', _.trim(line.substr(178, 62)));
    _.set(result, 'id_arquivo', params.fileId);
...
const parserRecord3 = (params) => {
    let result = {},
        { line, fileKey, lineNumber } = params,
        agencia = line.substr(53, 4),
        conta_corrente = line.substr(65, 5),
        ...
        cod_fluxo_caixa = line.substr(172, 4),
        desc_historico = _.trim(line.substr(176, 25)),
        nome_adquirente = _.first(desc_historico.split(' ')),
        nro_documento = line.substr(234, 6),
        set_adquirente = false,
    ...
    if (cod_fluxo_caixa == '0093') {
        set_adquirente = true;
    }
    if (cod_fluxo_caixa == '0069') {
        if (_.toNumber(nro_documento) > 0) {
            set_adquirente = true;
        }
    }
    if (set_adquirente == true) {
        _.set(adquirente, 'code', _.lowerCase(nome_adquirente));
        _.set(adquirente, 'name', _.capitalize(nome_adquirente));
        ponto_venda = _.first(desc_historico.match(/\d+/g));
        keyCommon.push(adquirente.code);
        keyCommon.push(sizedField(ponto_venda, 12));
        keyCommon.push(data_lancamento);
        conciliationType = 'creditcard'
    }
    if (cod_fluxo_caixa == '0038') {
        let branch = _.first(desc_historico.match(/\d+/g)),
            account = _.slice(desc_historico.match(/\d+/g), 1, 2);
        keyCommon.push('341');
        keyCommon.push(sizedField(agencia, 5));
        keyCommon.push(sizedField(conta_corrente, 10));
        keyCommon.push(data_lancamento);
        conciliationType = 'slip'
    }
    if ((cod_fluxo_caixa == '0071') && (_.includes(_.toUpper(desc_historico), 'DEB AUTOR') == true)) {
        let dtDebitoAutomatico = _.trim(_.first(desc_historico.match(/\d+/g)));
        if (_.isEmpty(dtDebitoAutomatico) == false) {
            keyCommon.push('341');
            keyCommon.push(sizedField(agencia, 5));
            keyCommon.push(sizedField(conta_corrente, 10));
            keyCommon.push(dtDebitoAutomatico);
            conciliationType = 'directDebit'
        }
    }
    if ((cod_fluxo_caixa == '0063') && (_.includes(_.toUpper(desc_historico), 'TRF TAR DE') == true)) {
        keyCommon.push('341');
        keyCommon.push(sizedField(agencia, 5));
        keyCommon.push(sizedField(conta_corrente, 10));
        keyCommon.push(data_lancamento);
        conciliationType = 'slipFee'
    }
    ...
    _.set(result, 'ident_lancamento', getIdTable('ident_lancamento', _.trim(line.substr(14, 1))));
    _.set(result, 'complemento1', _.trim(line.substr(15, 2)));
    _.set(result, 'tipo_inscricao', line.substr(17, 1));
    _.set(result, 'nro_inscricao', line.substr(18, 14));
    _.set(result, 'complemento2', _.trim(line.substr(32, 15)));
    _.set(result, 'cod_empresa', line.substr(47, 5));
    ...
    _.set(result, 'dig_conta_corrente', line.substr(71, 1));
    _.set(result, 'nome_empresa', _.trim(line.substr(72, 30)));
    _.set(result, 'uso_banco', _.trim(line.substr(102, 6)));
    _.set(result, 'natureza_lancamento', getIdTable('natureza_lancamento', _.trim(line.substr(108, 3))));
    _.set(result, 'tipo_complemento', line.substr(111, 2));
    _.set(result, 'banco_origem', _.trim(line.substr(113, 3)));
    _.set(result, 'agencia_origem', _.trim(line.substr(116, 5)));
    _.set(result, 'conta_origem', _.trim(line.substr(121, 12)));
    ...
    _.set(result, 'categoria_lancamento', getIdTable('categoria_lancamento', _.trim(line.substr(169, 3))));
    _.set(result, 'cod_fluxo_caixa', getIdTable('cod_fluxo_caixa', _.trim(cod_fluxo_caixa)));
    _.set(result, 'desc_historico', desc_historico);
    _.set(result, 'adquirente', adquirente);
    _.set(result, 'ponto_venda', ponto_venda);
    _.set(result, 'agencia_origem_rev', _.trim(line.substr(201, 4)));
    _.set(result, 'complemento6', _.trim(line.substr(205, 2)));
    _.set(result, 'conta_origem_rev', _.trim(line.substr(207, 5)));
    _.set(result, 'dig_conta_origem_rev', _.trim(line.substr(212, 1)));
    _.set(result, 'tipo_insc_emitente', _.trim(line.substr(213, 1)));
    _.set(result, 'nro_insc_emitente', _.trim(line.substr(214, 14)));
    _.set(result, 'complemento7', _.trim(line.substr(228, 6)));
...
const parserRecord5 = (params) => {
    ...
    _.set(result, 'complemento5', _.trim(line.substr(70, 1)));
    _.set(result, 'dig_conta_corrente', line.substr(71, 1));
    _.set(result, 'complemento6', _.trim(line.substr(72, 16)));
    _.set(result, 'complemento7', _.trim(line.substr(88, 54)));
    _.set(result, 'data_saldo', line.substr(146, 4) + '-' + line.substr(144, 2) + '-' + line.substr(142, 2));
    ...
    _.set(result, 'somatorio_futuro', Number(line.substr(212, 18))/100);
    _.set(result, 'complemento8', _.trim(line.substr(230, 10)));
    _.set(result, 'id_arquivo', params.fileId);

    params.totals.qtd_record5 += (result.qt_registros_lote - 2);
    params.totals.vl_record5 += (result.somatorio_debito + result.somatorio_credito + result.somatorio_futuro);
...
const parserRecord9 = (params) => {
    ...
    _.set(result, 'lote', line.substr(3, 4));
    _.set(result, 'registro', line.substr(7, 1));
    _.set(result, 'complemento1', line.substr(8, 9));
    ...
    _.set(result, 'complemento2', line.substr(35, 205));
    _.set(result, 'id_arquivo', params.fileId);
...
export const saveData = async (params) => {
    ...
    try {
        for (var i = 0; i < parsed.length; i++) {
            ...
            if (registro == '0') {
                await mongodb.cldr_itau_extrato_record0.upsert(filter, model);
            } else if (registro == '1') {
                await mongodb.cldr_itau_extrato_record1.upsert(filter, model);
            } else if (registro == '3') {
                await mongodb.cldr_itau_extrato_record3.upsert(filter, model);
            } else if (registro == '5') {
                await mongodb.cldr_itau_extrato_record5.upsert(filter, model);
            } else if (registro == '9') {
                await mongodb.cldr_itau_extrato_record9.upsert(filter, model);
            }
        }
    } catch (error) {
        throw error;
